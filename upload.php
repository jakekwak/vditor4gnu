...
for ($i = 0; $i < count($_FILES["file"]["name"]); $i++) {
	...
	if ($isUpload) {
		...
		// thanks to @dewoweb 
		if (!preg_match("/(jpe?g|gif|bmp|png|webp|svg|avif|wav|mp4|mp3|pdf)$/i", $filename_ext)) {   // check file extension 
			// error
			@unlink($tmp_name);
			...
			!stripos($mime_result, 'webp') &&
			!stripos($mime_result, 'avif') &&
			!stripos($mime_result, 'heif') &&
			!stripos($mime_result, 'svg') &&
			!stripos($mime_result, 'wav') &&
      !stripos($mime_result, 'mp4') &&
      !stripos($mime_result, 'mpeg') &&
      !stripos($mime_result, 'pdf') &&
			!stripos($mime_result, 'png')
		) {
			@unlink($tmp_name);
			array_push($errFiles, $name);
			array_push($error, "Mime 타입이 다릅니다.");
		} else if (
      !(stripos($mime_result, 'svg') || 
        stripos($mime_result, 'wav') ||
        stripos($mime_result, 'mp4') ||
    ...
    ) {	// check image resolution, if resolutions is null, return fail 
			@unlink($tmp_name);
			array_push($errFiles, $name);
...
if ($error) {
	...
	// $msg = "파일이 성공적으로 업로드되었습니다.".$mime_result; // mime_result 체크용
